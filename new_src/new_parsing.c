...
/*   Updated: 2024/02/03 15:47:37 by hclaude          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_free_mega_split(char ***split_map_content, t_fdf *map_data)
{
    ...
    {
        int x_pos = 0;
        while (x_pos < map_data->xmax && split_map_content[y_pos][x_pos] != NULL)
...
int ft_split_map_content(char **map_content, t_fdf *map_data)
{
	...
	// for (int i = 0; i < map_data->ymax; i++) {
    //     for (int j = 0; j < map_data->xmax; j++) {
    //         printf("%s ", split_map_content[i][j]);  // Imprimer l'élément actuel
    //     }
	// }
    // printf("\n");
...
int ft_init(t_fdf *map_data, char *filepath)
{
	...
	// printf("fd = %d && ymax = %d", fd, map_data->ymax);
	return (1);
}
